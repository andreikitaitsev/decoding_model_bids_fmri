...
def mps2spectr(mps, phases, **kwargs):
    ...
    for time in range(mps.shape[0]):
        ...
        spectr_iter = np.fft.ifft2(np.fft.ifftshift(mps_iter),**kwargs)
...
def mps2audio(mps_path, stim_param_path, phase_path):
    '''Converts mps into audio and saves in under filename.
    Inputs:
        mps_path -str
        stim_param_path - str
        phase_path - str, path to phase array
    Outputs:
...
#        mps2audio(mps_path, stim_path, fname, 'orig')
    ...
    mps_filepath = '/data/akitaitsev/data1/decoding_data5/audio/task-aomovie_run-1_stim.tsv.gz'
    phases_filepath='/data/akitaitsev/data1/decoding_data5/audio/mps_phases_run-1.tsv.gz'
    param_filepath='/data/akitaitsev/data1/decoding_data5/audio/task-aomovie_run-1_stim_parameters.json'
    out_file = '/data/akitaitsev/data1/decoding_data5/audio/reconstr_orig_run-1.wav'
    
    audio = mps2audio(mps_filepath, param_filepath, phases_filepath)
    soundfile.write(out_file, audio, 20000)
